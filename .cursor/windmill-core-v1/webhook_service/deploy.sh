...
HEALTH_URL="${WEBHOOK_HEALTH_URL:-http://localhost:8001/health}"
...
do_restart() {
    ...
    write_status "running" "$CURRENT_STEP" "null" "$$"
    log "[4/5] Перезапуск webhook_service…"
    if systemctl is-enabled biretos-webhook.service &>/dev/null; then
        systemctl restart biretos-webhook.service
    else
        pkill -f 'python.*main.py.*webhook_service' || true
        (nohup python3 main.py > webhook_service.log 2>&1 &) >/dev/null 2>&1
    fi
    sleep 3
}
...
do_verify() {
    CURRENT_STEP="verify"
    write_status "running" "$CURRENT_STEP" "null" "$$"
    log "[5/5] Проверка health endpoint…"
    if curl -sf "$HEALTH_URL" >/dev/null; then
        write_status "completed" "$CURRENT_STEP" "null" "$$"
        rm -f "$STARTED_AT_FILE"
        log "✅ Деплой webhook_service завершён успешно"
    else
        write_status "failed" "$CURRENT_STEP" "\"Health endpoint не отвечает\"" "$$"
        exit 1
    fi
}
