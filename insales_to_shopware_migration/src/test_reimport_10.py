...
def verify_product(client, product_number, expected_base_price, expected_marketplace_price):
    """Проверяет товар после импорта"""
    try:
        ...
        try:
            ...
            if response.get("total") and response.get("data"):
                ...
                if found_pn == product_number:
                    product_id = product.get("id")
        except:
            pass
        
        # Если не нашли через Search API, пробуем через find_product_by_number
        if not product_id:
            product_id = client.find_product_by_number(product_number)
        
        if not product_id:
            return False, "Товар не найден", None, None, None
...
def main():
    ...
    for item in products_data:
        ...
        ok, status, found_base, found_marketplace, details = verify_product(
            client, sku, expected_base, expected_marketplace
        )
