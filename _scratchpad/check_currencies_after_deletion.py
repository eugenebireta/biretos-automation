"""
Проверка валют в Shopware после удаления лишних валют
"""
import sys
...
print("=" * 60)
print("ПРОВЕРКА ВАЛЮТ ПОСЛЕ УДАЛЕНИЯ")
print("=" * 60)
...
try:
    ...
    if len(currencies) == 1:
        print("✓ Отлично! Осталась только одна валюта")
    elif len(currencies) == 0:
        print("⚠ ВНИМАНИЕ: Валют не найдено! Это критическая ошибка!")
    else:
        print(f"⚠ Найдено {len(currencies)} валют - должны быть удалены лишние")
    ...
    for curr in currencies:
        ...
        try:
            ...
            if iso_code and iso_code.upper() == 'RUB':
                print(f"    ✓ Это RUB - правильно!")
            else:
                print(f"    ⚠ Это не RUB! Должна быть удалена.")
    ...
    for sc in sales_channels:
        ...
        try:
            ...
            if currency_id:
                ...
                curr_attrs = curr_data.get("attributes", {})
                iso_code = curr_attrs.get("isoCode")
                print(f"    Currency: {iso_code}")
                
                if iso_code and iso_code.upper() == 'RUB':
                    print(f"    ✓ Использует RUB - правильно!")
                else:
                    print(f"    ⚠ Использует {iso_code} вместо RUB!")
        except Exception as e:
            print(f"  ⚠ Ошибка получения деталей Sales Channel {sc_id}: {e}")
except Exception as e:
    print(f"Ошибка получения Sales Channels: {e}")

# Проверяем метод получения валюты
print("\n3. ПРОВЕРКА МЕТОДА ПОЛУЧЕНИЯ ВАЛЮТЫ:")
print("-" * 60)
try:
    ...
    if sales_channel_currency:
        ...
        if iso_code and iso_code.upper() == 'RUB':
            ...
            print(f"  ⚠ Проблема: используется {iso_code} вместо RUB")
except Exception as e:
    print(f"Ошибка: {e}")
    import traceback
    traceback.print_exc()
