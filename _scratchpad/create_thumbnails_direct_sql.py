"""
Создание thumbnail sizes напрямую через SQL команды.
"""
import sys
import paramiko
import uuid
...
print("=" * 60)
print("СОЗДАНИЕ THUMBNAIL SIZES (НАПРЯМУЮ)")
print("=" * 60)
...
try:
    ...
    create_table = "CREATE TABLE IF NOT EXISTS thumbnail_size (id BINARY(16) NOT NULL, width INT(11) NOT NULL, height INT(11) NOT NULL, created_at DATETIME(3) NOT NULL, updated_at DATETIME(3) NULL, PRIMARY KEY (id), UNIQUE KEY uniq_width_height (width, height)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"
    
    command1 = f"docker exec {container_name} mysql -h 127.0.0.1 -u root -proot shopware -e \"{create_table}\" 2>&1 | grep -v Warning"
    ...
    # Шаг 2: Создаём размеры по одному
    ...
    for width, height in thumbnail_sizes:
        ...
        # Вставляем
        insert_sql = f"INSERT IGNORE INTO thumbnail_size (id, width, height, created_at, updated_at) VALUES (UNHEX('{size_id_hex}'), {width}, {height}, '{now}', '{now}');"
    ...
    if output and "ERROR" not in output:
        ...
        print(f"   ⚠️  Не удалось получить список: {error[:200] if error else 'нет вывода'}")
