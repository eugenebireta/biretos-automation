...
(обновлено 2026-03-16: v1.9.0 patch applied — Gold SKU Engine, R4, Content Gen,
Marketplace Adapt, Observability, Rollback Matrix, Aged Stock, TD-026..TD-038)
...
═══════════════════════════════════════════════════════
  ...
  1. Открыть чат с Claude, скинуть PROJECT_DNA.md + этот файл
  2. Сказать "следующая задача"
  3. Claude даёт промпт для Cursor с правильным pipeline
...
═══════════════════════════════════════════════════════
  ...
  Условие открытия: Iron Fence (Этап 5.5 MVP) стабилен,
  CI зелёный на всех трёх grep-проверках.

  Каждый Revenue трек = отдельная ветка feat/rev-<n>-*
  Все Revenue треки = Tier-3 адаптеры (см. PROJECT_DNA §6)


R1 — MASS CATALOG PIPELINE                      🟡 SEMI-CRITICAL
MVP срок: 3-7 дней
  ...
  R1.4 Фото: поиск + скачивание → нормализация → загрузка → URL
  R1.5 Idempotency + retries + audit log
  R1.6 Тесты в CI

  Hard Stop: НЕ auto-delete фото, НЕ fuzzy match, НЕ ML.
  Метрика: 500 SKU опубликованы с фото без ручного вмешательства.
...
═══════════════════════════════════════════════════════
  ...
  ЧАСТЬ I — CORE FOUNDATION          [~3-4 мес]
    ...
    Этап 4.5: Observability Strategy  🟡 → tracing + diagnostics  ← NEW
    Этап 5:   Pydantic Validation     🔴 → runtime contracts
    Этап 5.5: Iron Fence              🟡 → Core защищён
    ...
    Этап 8.1: Local AI Setup (+8.1.6) 🟡 → AI независим          ← EXPANDED
    Этап SS-1: DataSheet Foundation   🟡 → Intelligence фундамент
    ── STOP RULE v1: можно заморозить ──

  REVENUE TRACKS
    R1: Mass Catalog Pipeline         🟡 → 500 SKU
    R2: Telegram Export               🟢 → выгрузка за 30 сек
    R3: Lot Analyzer v2               🟡 → score + research pack  ← EXPANDED
    R4: Anchor Buyer Liquidation      🟡 → core payback ≤30 дней  ← NEW
...
═══════════════════════════════════════════════════════
  ...
  5. Master Plan (v1.9.0) — конституция.
