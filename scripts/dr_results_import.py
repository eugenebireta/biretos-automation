...
def _build_result_json(row: dict[str, str], provider: str, source_file: str) -> Optional[dict]:
    ...
    title_ru = category if not _is_not_found(category) else ""
    ...
    return {
        ...
        "final_recommendation": {
            "identity_confirmed": not _is_not_found(category),
            "brand": "unknown",
            "title_ru": title_ru,
...
def import_file(
    ...
    # Find the first pipe table header row with "| # |" and "| PN |"
    section_lines = _find_table_section(text, "table 1")
    # Try to find the actual table header
    ...
    if table1_start is None:
        # Try without the "# |" prefix — might be "| # | PN |" style
        for i, line in enumerate(all_lines):
    ...
    if table1_start is not None:
        rows = _parse_table_rows(all_lines[table1_start:])
    else:
        rows = []
