...
## Версия: 2.4 (Corrected Execution Track)
## Дата: 2026-04-03
...
Статус: **Core заморожен.** В repo уже есть Revenue и governed-execution артефакты,
но текущий corrective execution order зафиксирован отдельно и имеет приоритет над
автоматическим продолжением Stage 8.1 / runtime-expansion треков.
...
## 1c. Current Corrective Execution Order

До отдельного owner-reopen действуют следующие правила исполнения:
1. Сначала **Phase 0 — loss prevention / safety / repo integrity**
2. Затем **Phase 1 — governance codification** в authoritative docs / rules / configs
3. Наличие Stage 8.1 кода, артефактов или старых PR в repo **не** открывает
   Stage 8.1 автоматически
4. Stage 8.1 трактуется как **local review fabric / shadow gate / disagreement logging**,
   а не как owner authority и не как разрешение на новый production control path
5. Любой новый runtime wiring для local reviewer / shadow gate требует
   отдельного batch и отдельного owner instruction
